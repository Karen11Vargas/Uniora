...
export interface ServiceRequest {
  ...
  title: string;
  description: string;
  category: 'mantenimiento' | 'seguridad' | 'administrativo' | 'otro';
...
export function createApiClient(baseUrl = API_URL) {
  ...
  return {
    ...
    // Solicitudes
    getRequests: () => request<ServiceRequest[]>(withBase('/requests')),

    createRequest: (payload: Omit<ServiceRequest, 'id' | 'status' | 'createdAt'>) =>
      request<ServiceRequest>(withBase('/requests'), {
        method: 'POST',
        body: JSON.stringify(payload)
      }),

    updateRequestStatus: (id: string, status: ServiceRequest['status']) =>
      request<ServiceRequest>(withBase(`/requests/${id}/status`), {
        method: 'PATCH',
        body: JSON.stringify({ status })
      })
  } as const;
