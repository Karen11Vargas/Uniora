...
let serviceRequests = [
  {
    id: uuidv4(),
    title: 'Fuga de agua en torre 2',
    description: 'Se escucha goteo constante en la bajante del piso 7.',
    category: 'mantenimiento',
    status: 'en-progreso',
    createdAt: new Date().toISOString(),
    requestedBy: 'Propietario - Torre 2 Apt 702'
  },
  {
    id: uuidv4(),
    title: 'Reporte de vehículo sospechoso',
    description: 'Camioneta blanca estacionada sin permiso en parqueadero de visitantes.',
    category: 'seguridad',
    status: 'abierto',
    createdAt: new Date().toISOString(),
    requestedBy: 'Guardia principal'
  }
];
...
app.post('/api/requests', (req, res) => {
  const { title, description, category, requestedBy } = req.body || {};

  if (!title || !description || !category) {
    return res.status(400).json({ message: 'Datos incompletos para la solicitud' });
  }
